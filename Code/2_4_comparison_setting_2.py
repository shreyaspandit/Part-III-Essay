...
def generate_data(a, b, n):
    ...
    s = (np.abs(x) + 2) * 0.5 * (np.sin(2*x) + 1.5)
...
ax1.legend()
fig1.savefig("figures/2_4_heteroscedastic_RF.png")
plt.show()
...
ax2.legend()
fig2.savefig("figures/2_4_heteroscedastic_LW.png")
plt.show()
...
ax3.legend()
fig3.savefig("figures/2_4_heteroscedastic_CQR.png")
plt.show()
...
ax3.plot(z[:-1], local_cov_split, color="blue", label="Absolute residual score")
ax3.plot(z[:-1], local_cov_W, color="red", label="Locally weighted residual score")
ax3.plot(z[:-1], local_cov_Q, color="orange", label="Conformalised quantile regression")
ax3.legend()
fig3.savefig("figures/2_4_local_cov_setting2.png")
plt.show()